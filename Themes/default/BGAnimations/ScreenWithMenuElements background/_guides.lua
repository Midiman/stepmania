...
local screen_grid = Def.ActorFrame {
	...
	LoadActor(THEME:GetPathG("_texture","grid")) .. {
		InitCommand=cmd(zoomto,SCREEN_WIDTH,SCREEN_HEIGHT),
		OnCommand=cmd(diffusealpha,0.25;customtexturerect,0,0,SCREEN_WIDTH/grid_size_large,SCREEN_HEIGHT/grid_size_large)
	}
}
...
local menu_region = Def.ActorFrame {
	...
	Def.Quad {
		...
		OnCommand=cmd(diffuse,Color.Green;diffusealpha,0.5;zoomto,MENU_CENTER_X,MENU_HEIGHT)
	},
	Def.Quad {
		InitCommand=cmd(xy,MENU_CENTER_X/2,MENU_CENTER_Y),
		OnCommand=cmd(diffuse,Color.Green;diffusealpha,0.5;zoomto,MENU_CENTER_X *0.925,MENU_HEIGHT *0.925)
	}
}
...
local menu_center = Def.ActorFrame {
	-- Menu Horizontal
	Def.Quad {
		InitCommand=cmd(xy,MENU_CENTER_X,MENU_CENTER_Y;zoomto,MENU_WIDTH,2),
		OnCommand=cmd(diffuse,Color.Yellow;diffusealpha,0.5)
	},
	-- Menu Vertical
	Def.Quad {
		InitCommand=cmd(xy,MENU_CENTER_X,MENU_CENTER_Y;zoomto,2,MENU_HEIGHT),
		OnCommand=cmd(diffuse,Color.Yellow;diffusealpha,0.5)
	},
}


local menu_43 = Def.ActorFrame {
	-- Left
	Def.Quad {
		InitCommand=cmd(xy,SCREEN_CENTER_X - 320,SCREEN_CENTER_Y;zoomto,2,SCREEN_HEIGHT),
		OnCommand=cmd(diffuse,Color.Red;diffusealpha,0.5)
	},
	-- Right
	Def.Quad {
		InitCommand=cmd(xy,SCREEN_CENTER_X + 320,SCREEN_CENTER_Y;zoomto,2,SCREEN_HEIGHT),
